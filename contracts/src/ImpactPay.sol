...
contract ImpactPay is Pausable, Ownable, ReentrancyGuard {
    ...
    function createScholarshipGoal(
        ...
        if (!level3Verified[_msgSender()]) revert Level3Required();
        _createGoal(
            targetAmount, 
            hex"",
            bytes(description),
    ...
    function restriction(address target, bool status) public onlyOwner returns(bool) {
    ...
    function claimScholarshipFunds(uint256 goalId, address recipient) external notRestricted(_msgSender()) whenNotPaused nonReentrant returns(bool) {
